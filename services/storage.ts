...
export const storageService = {
  ...
  async _fetchFullData(): Promise<AppData> {
    ...
    let localUsers: any[] = [];
    let localTracks = DEFAULT_TRACKS;
    let localWeeklySchedule = {};
    let localDailyUsedLastFmAccounts = {};

    try {
      if (usersStr) localUsers = JSON.parse(usersStr) || [];
      if (tracksStr) localTracks = JSON.parse(tracksStr) || DEFAULT_TRACKS;
      if (scheduleStr) localWeeklySchedule = JSON.parse(scheduleStr) || {};
      if (usedAccStr)
        localDailyUsedLastFmAccounts = JSON.parse(usedAccStr) || {};
    } catch (e) {
    ...
    const localData = {
      ...
      dailyUsedLastFmAccounts: localDailyUsedLastFmAccounts,
    };

    // 1. CLOUD MODE (FIREBASE)
    try {
      const docRef = doc(db, "appData", "main");
      const docSnap = await this._withTimeout(getDoc(docRef), 15000);

      if (docSnap.exists()) {
        const record = docSnap.data();

        let fbUsers = Array.isArray(record.users) ? record.users : [];
        const mergedUsersMap = new Map();
        localUsers.forEach((u) => mergedUsersMap.set(u.id, u));
        fbUsers.forEach((u) => mergedUsersMap.set(u.id, u));
        ...
        const mergedData = {
          ...
          tracks: Array.isArray(record.tracks)
            ? record.tracks
            : record.tracks || localTracks,
          spotifyPlaylistId:
            record.spotifyPlaylistId || localData.spotifyPlaylistId,
          weeklySchedule: record.weeklySchedule || localWeeklySchedule,
          adminPin: record.adminPin || localData.adminPin,
          dailyUsedLastFmAccounts:
            Object.keys(record.dailyUsedLastFmAccounts || {}).length > 0
              ? record.dailyUsedLastFmAccounts
              : localDailyUsedLastFmAccounts,
        };

        // Auto-seed Firebase so we don't lose the local fallback data on next fetches from other devices
        try {
          if (typeof db !== "undefined") {
            setDoc(doc(db, "appData", "main"), mergedData).catch(() => {});
          }
        } catch (e) {}
    ...
    // Auto-seed Firebase so we don't lose the local fallback data on next fetches from other devices
    try {
      if (typeof db !== "undefined" && localUsers.length > 0) {
  ...
  async _saveFullData(data: AppData): Promise<void> {
    ...
    try {
      const docRef = doc(db, "appData", "main");
      await this._withTimeout(setDoc(docRef, data), 15000);
  ...
  _updateLocalCache(data: AppData) {
    ...
    if (data.spotifyPlaylistId)
      localStorage.setItem(STORAGE_KEY_SPOTIFY, data.spotifyPlaylistId);
    if (data.weeklySchedule)
      localStorage.setItem(
        "streamguard_schedule",
        JSON.stringify(data.weeklySchedule),
      );
    if (data.adminPin)
      localStorage.setItem("streamguard_admin_pin", data.adminPin);
    if (data.dailyUsedLastFmAccounts)
      localStorage.setItem(
        "streamguard_used_accounts",
        JSON.stringify(data.dailyUsedLastFmAccounts),
      );
  },
  ...
  async registerUser(newUser: User): Promise<User> {
    ...
    if (
      users.some(
        (u) =>
          u.appUsername.toLowerCase() === newUser.appUsername.toLowerCase(),
      )
    ) {
  ...
  async updateUserCheckIn(
    userId: string,
    dateString: string,
    usedLastFmUsername: string | string[],
  ): Promise<User> {
    const data = await this._fetchFullData();
    const users = Array.isArray(data.users) ? data.users : [];
    ...
    if (usedLastFmUsername) {
      const usernames = Array.isArray(usedLastFmUsername)
        ? usedLastFmUsername
        : [usedLastFmUsername];
    ...
    const newUsers = users.map((u) => {
      if (u.id === userId) {
        ...
        const newLastCheckIn =
          dateString === todayStr ? dateString : u.lastCheckInDate;
    ...
    await this._saveFullData({
      ...data,
      users: newUsers,
      dailyUsedLastFmAccounts: dailyUsedMap,
    });
    return updatedUser!;
  },

  async isLastFmAccountUsed(
    dateString: string,
    username: string,
  ): Promise<boolean> {
